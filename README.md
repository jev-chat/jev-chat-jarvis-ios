...
1. 设置 → 通用 → 键盘 → 键盘 → **添加新键盘** → 选「秒回键盘」
2. 回到键盘列表，点「秒回键盘」→ 打开 **允许完全访问**（联网 + 读剪贴板必须，微信里搜狗键盘也是同款开关）
3. 打开「秒回」App →「模型」页：选个预设（智谱 `glm-4-flash` 免费 / DeepSeek 最快 / OpenRouter 一个 key 全模型），填 Key，点「测试连接」
4. 到「试一试」页跑一条消息，通了就是全通了

### 3. 日常使用（以微信为例）

1. 长按对方那条消息 → **复制**
2. 输入框获得焦点，键盘切到秒回（地球键切换）→ 点 **「分析剪贴板」**
